...
images.use(
  async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    const filePath = `${
      path.join(__dirname, '../../../images/thumb/') + req.query.filename
...
images.get(
  ...
  async (
    ...
    const filePath = `${
      path.join(__dirname, '../../../images/thumb/') + filename
    ...
    try {
      ...
      if (!res.headersSent) {
        ...
        res
          .status(401)
          .send(
            'We have failed to process your image, please check you filename and image sizes'
          );
      }
    }
  }
